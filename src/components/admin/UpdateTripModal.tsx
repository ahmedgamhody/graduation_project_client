import { SquarePen, XCircle, Search } from "lucide-react";
...
import { updateTripByAdmin } from "../../utils/api";
import { queryClient } from "../../main";

export default function UpdateTripModal({
  handleCloseUpdateTripModal,
  tripDetails,
}: {
  handleCloseUpdateTripModal: () => void;
  tripDetails: TTripDetails | null;
}) {
  ...
  const {
    ...
    defaultValues: {
      ...
      programName: tripDetails?.programName || "",
      trips_Places: tripDetails?.tripPlaces?.map((place) => place.name) || [],
    },
  });

  // Initialize selected places from tripDetails
  useEffect(() => {
    if (tripDetails?.tripPlaces) {
      const placeNames = tripDetails.tripPlaces.map((place) => place.name);
      setSelectedPlaces(placeNames);
      setValue("trips_Places", placeNames);
      setValue("programName", tripDetails.programName || "");
      setValue("description", tripDetails.description || "");
      setValue("price", tripDetails.price || 0);
      setValue("days", tripDetails.days || 0);
    }
  ...
  const handlePlaceSelect = async (place: string) => {
    ...
    setValue("trips_Places", newSelectedPlaces);
    await trigger("trips_Places");
    setPlaceSearchTerm("");
    setShowPlaceDropdown(false);
  };
  ...
  const onSubmit = async (data: UpdateTripFormData) => {
    if (!tripDetails?.name) return;

    try {
      setIsSubmitting(true);
      const requestData = {
        ...data,
        trips_Places: selectedPlaces,
      };

      await updateTripByAdmin(token, tripDetails.name, requestData);
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900">Update Trip</h3>
              <p className="text-sm text-gray-500">Trip: {tripDetails.name}</p>
            </div>
          </div>
